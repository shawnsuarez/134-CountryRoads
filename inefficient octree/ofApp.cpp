...
//  Have Fun !!
//
//  Student Name:   David Bui
//  Date: 4/22/2019

// Octree drawn with 5 levels
...
//	ofSetWindowShape(1024, 768);
	...
	//mars.loadModel("geo/cornMoon.obj");
	mars.setScaleNormalization(false);

	// reverse the model
	//mars.setRotation(0, 180, 0, 0, 1);
	gui.setup();
	gui.add(octreeLevels.setup("Octree Levels", MAX_OCTREE_LEVEL, 0, MAX_OCTREE_LEVEL));
	octree.create(mars.getMesh(0), octreeLevels);
...
//	cout << ofGetFrameRate() << endl;
	...
	if (bWireframe) {                    // wireframe mode  (include axis)
		...
		mars.drawWireframe(); // Draw the frame
		if (bRoverLoaded) {
			rover.drawWireframe();
	...
	// drawing the octree
	octree.draw(octree.root, MAX_OCTREE_LEVEL, 0);
...
void ofApp::mousePressed(int x, int y, int button) {
	...
	// hit bounding box
	glm::vec3 sw = theCam->screenToWorld(glm::vec3(x, y, 0));
	glm::vec3 dir = sw - theCam->getPosition();
	glm::vec3 dirNormalized = glm::normalize(dir);
	glm::vec3 origin = theCam->getPosition();
	Ray ray = Ray(Vector3(origin.x, origin.y, origin.z), Vector3(dirNormalized.x, dirNormalized.y, dirNormalized.z));


	float startTime = ofGetElapsedTimeMillis();
	float timeElapsed;
	if (octree.intersect(ray, octree.root, octree.selectedPoint))
	{

		drawIntersectPoint = true;
		int point = octree.selectedPoint.points[0];
		intersectPoint = mars.getMesh(0).getVertex(point);
		timeElapsed = (ofGetElapsedTimeMillis() - startTime) / 1000;
		cout << timeElapsed << " seconds to select a point" << endl;
	}


	bool hit = boundingBox.intersect(ray, 0, 10000);
...
void ofApp::initLightingAndMaterials() {
	...
	glEnable(GL_LIGHT0);
	//glEnable(GL_LIGHT1); // Make brighter
